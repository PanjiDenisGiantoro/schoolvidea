@extends('layouts.app')
@section('title', isset($payroll_components) ? (isset($show) && $show ? 'Lihat Komponen' : 'Edit Komponen') : 'Tambah
    Komponen')
@section('content')
    @include('partials.page-title', [
        'title' => isset($payroll_components)
            ? (isset($show) && $show
                ? 'Lihat Data'
                : 'Edit Data')
            : 'Tambah Data',
    ...
    <div class="card">
        <div class="card-body">
            <form id="componentForm" method="POST"
                ...
                <div class="row p-4">
                    <div class="col-md-6">
                        <x-input-field type='text' name='name' label='Nama Komponen' placeholder='Masukkan Nama Komponen'
                            icon='bx bx-unit' :value="old('name', $payroll_components->name ?? '')" required />
                    </div>
                    <div class="col-md-6">
                        <x-input-field type="text" name="price_display" label="Nilai Komponen"
                        ...
                        <input type="hidden" name="price" id="price_hidden"
                            value="{{ old('price', $payroll_components->price ?? '') }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" id="status" class="form-select">
                            <option value="1"
                                {{ old('status', $payroll_components->status ?? '') == '1' ? 'selected' : '' }}>Aktif
                            </option>
                            <option value="0"
                                {{ old('status', $payroll_components->status ?? '') == '0' ? 'selected' : '' }}>Tidak Aktif
                            </option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label for="description" class="form-label">Keterangan</label>
                        <textarea name="description" id="description" style=" height: 45px; font-size: 0.95rem;" class="form-control"
                            rows="2">{{ old('description', $payroll_components->description ?? '') }}</textarea>
                    </div>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            @if (isset($show) && $show)
                document.querySelectorAll(
                    '#componentForm input, #componentForm select, #componentForm textarea, #componentForm button[type="submit"]'
                    ).forEach(el => {
                    el.disabled = true;

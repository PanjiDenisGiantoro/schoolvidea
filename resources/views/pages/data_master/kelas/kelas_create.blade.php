...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form id="kelasForm" action="{{ isset($kelas) ? route('kelas.update', $kelas->id) : route('kelas.store') }}"
                method="POST">
                @csrf
                @if (isset($kelas))
                    @method('PUT')
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        <x-input-field type="text" name="kode_kelas" label="Kode Kelas" placeholder="Masukkan Kode Kelas"
                            icon="bx bx-book" :value="old('kode_kelas', $kelas->kode_kelas ?? '')" required />
                    </div>
                    <div class="col-md-4">
                        <x-input-field type="text" name="nama_kelas" label="Nama Kelas" placeholder="Masukkan Nama Kelas"
                            icon="bx bx-book" :value="old('nama_kelas', $kelas->nama_kelas ?? '')" required />
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="officer_id" id="officer_id" class="form-select" data-choices
                            ...
                            @foreach ($wali as $w)
                                <option value="{{ $w->id }}"
                                    {{ old('officer_id', $kelas->officer_id ?? '') == $w->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="status" id="status" class="form-select">
                            ...
                            <option value="1" {{ old('status', $kelas->status ?? '') == 'Aktif' ? 'selected' : '' }}>
                                Aktif</option>
                            <option value="0"
                                {{ old('status', $kelas->status ?? '') == 'Tidak Aktif' ? 'selected' : '' }}>Non Aktif
                            </option>
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label for="jurusan_id">Jurusan</label>
                        <select name="jurusan_id" id="jurusan_id" class="form-select" data-choices
                            data-choices-sorting-false @if (isset($show) && $show) disabled @endif>
        ...
        @if (isset($show) && $show)
            document.addEventListener('DOMContentLoaded', function() {
                const formElements = document.querySelectorAll(
                    '#kelasForm input, #kelasForm textarea, #kelasForm select, #kelasForm button[type="submit"]'
                );

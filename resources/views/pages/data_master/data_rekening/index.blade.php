        ...
        <div class="card-body">
            <div class="row g-5">
                <div class="col-lg-12 d-flex justify-content-between align-items-center mb-3">
                    <h5 class="card-title mb-0">List Data Rekening</h5>
                    <a href="{{ url('data-rekening/create') }}" class="btn btn-primary">
                ...
                <div class="col-lg-12 mb-3">
                    <form method="GET" action="{{ route('data-rekening.index') }}" class="row g-3">
                        ...
                        <div class="col-md-{{ auth()->user()->unit_id === null ? '7' : '10' }}">
                            <input type="text" name="search" class="form-control p-3"
                                placeholder="Cari Rekening (Nama, Kode, Unit...)" value="{{ request('search') }}">
                        </div>
                        <div class="col-md-2">
                ...
                <table id="datatable" class="table-bordered table-striped table text-center align-middle text-nowrap">
                    ...
                    <tbody>
                        @forelse($datarekening as $index => $item)
                            <tr>
                                <td>{{ $datarekening->firstItem() + $index }}</td>
                                <td>{{ $item->unit->nama_unit }}</td>
                                <td>{{ $item->account_code }}</td>
                                <td>{{ $item->account_name }}</td>
                                <td>{{ $item->owner_name }}</td>
                                <td>{{ $item->akun ? ( $item->akun->kode_akun.' - '.$item->akun->nama_akun) : '-' }}</td>
                                <td>
                                    @if ($item->status == 1)
                                ...
                                <td>
                                    <div class="d-flex gap-3">
                                        ...
                                        <a href="{{ route('data-rekening.edit', $item->id ?? '') }}"
                                            class="link-warning text-muted">
                                            <i class="ri-edit-line fs-20 align-middle"></i>
                                            Edit
                                        </a>
                                        <a href="{{ route('data-rekening.destroy', $item->id ?? '') }}"
